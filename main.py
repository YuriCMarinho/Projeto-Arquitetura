...
def executar_projeto():
    ...
    e1_out = os.path.join(base_dir, 'Etapa_1', 'log_etapa1.txt')
    try:
        rodar_etapa1(e1_in, e1_out)
        print(f"OK: Log gerado em Etapa_1/log_etapa1.txt")
    ...
    e21_out = os.path.join(base_dir, 'Etapa_2', 'log_etapa2_tarefa1.txt')

    try:
        rodar_tarefa1(e21_in, e21_out) 
        print(f"OK: Log gerado em Etapa_2/log_etapa2_tarefa1.txt")
    except Exception as e:
    ...
    e2_log = os.path.join(base_dir, 'Etapa_2', 'log_etapa2_tarefa2.txt')
    
    try:
        maquina = Mic1Datapath()
        maquina.executar(e2_prog, e2_regs, e2_log)
        print(f"OK: Log gerado em Etapa_2/log_etapa2_tarefa2.txt")
    except Exception as e:
