...
def compilar_ijvm(arquivo_ijvm, arquivo_micro):
    """Lê instruções IJVM de alto nível e converte para microinstruções de 23 bits."""
    microinstrucoes = []
    
    if not os.path.exists(arquivo_ijvm):
        print(f"Arquivo {arquivo_ijvm} não encontrado. Crie para testar o tradutor.")
        return

    with open(arquivo_ijvm, 'r') as f:
        for linha in f:
            instrucao = linha.strip().split()
            if not instrucao: continue
            
            op = instrucao[0].upper()
            
            if op == 'DUP':
                microinstrucoes.append("00110101000001001000100") # MAR = SP = SP + 1
                microinstrucoes.append("00010100000000010100111") # MDR = TOS; wr
                
            elif op == 'ILOAD':
                x = int(instrucao[1])
                microinstrucoes.append("00010100100000000000101") # H = LV
                for _ in range(x):
                    microinstrucoes.append("00111001100000000000000") # H = H + 1
                microinstrucoes.append("00111000000000001010000") # MAR = H; rd
                microinstrucoes.append("00110101000001001100100") # MAR = SP = SP + 1; wr
                microinstrucoes.append("00010100001000000000000") # TOS = MDR
                
            elif op == 'BIPUSH':
                byte_val = int(instrucao[1])
                byte_bin = f"{byte_val & 0xFF:08b}"
                microinstrucoes.append("00110101000001001000100") # SP = MAR = SP + 1
                # A instrução FETCH especial recebe o valor nos 8 primeiros bits
                microinstrucoes.append(f"{byte_bin}000000000110000") # fetch (caso especial)
                microinstrucoes.append("00111000001000010100000") # MDR = TOS = H; wr
                
    with open(arquivo_micro, 'w') as f:
        for mi in microinstrucoes:
            f.write(f"{mi}\n")
...
if __name__ == "__main__":
    ...
    arq_ijvm = os.path.join(base, 'instrucoes_etapa3_tarefa2.txt')
    arq_micro = os.path.join(base, 'microinstrucoes_etapa3_tarefa2.txt')
    arq_regs = os.path.join(base, 'registradores_etapa3_tarefa2.txt')
    arq_mem = os.path.join(base, 'dados_etapa3_tarefa2.txt')
    arq_log = os.path.join(base, 'log_etapa3_tarefa2.txt')
    
    compilar_ijvm(arq_ijvm, arq_micro)
    
    Mic1Datapath().executar(arq_micro, arq_regs, arq_mem, arq_log)
    print(f"Execução da Tarefa 2 finalizada! Verifique o arquivo: {os.path.basename(arq_log)}")